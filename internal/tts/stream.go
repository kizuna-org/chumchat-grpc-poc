...
type TextToSpeechStream struct {
...
func NewTextToSpeechStream(onResponse func(*texttospeechpb.StreamingSynthesizeResponse) error) (*TextToSpeechStream, error) {
...
func (tts *TextToSpeechStream) Close() error {
	return tts.client.Close()
}

func (tts *TextToSpeechStream) Start() error {
	go func() {
		for {
			select {
			case <-tts.ctx.Done():
...
func (tts *TextToSpeechStream) Stop() error {
	tts.ctx.Done()
...
func (tts *TextToSpeechStream) Send(text string) error {
